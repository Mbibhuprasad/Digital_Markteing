...
export default function Hero() {
  ...
  // Floating animation variants for services
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 grid md:grid-cols-2 mt-8 gap-12 items-center text-white w-full">
          ...
          {showFirst ? (
            <>
              ...
              <h1 className="text-4xl md:text-4xl font-extrabold leading-snug mt-4">
                ...
                <motion.span
                  key={currentText}
                  className="text-[#8BD005] md:text-6xl inline-block"
                  variants={container}
                  initial="hidden"
                  animate="visible"
                >
              ...
              <div className="mt-8 flex w-full max-w-lg rounded-xl overflow-hidden bg-white/10 backdrop-blur-md">
                ...
                />
                <button className="bg-[#8BD005] hover:bg-[#7BBE04] px-6 py-3 font-semibold">
                  Get SEO Report
                </button>
              </div>
            </>
        ...
        {/* Right Content with Services Grid */}
        <motion.div
        ...
        >
          ...
          >
            {Object.entries(services).map(([key, service], index) => (
              <motion.div
                key={key}
                className="bg-white/10 backdrop-blur-md rounded-xl p-4 text-center cursor-pointer border border-white/20 hover:border-[#8BD005] transition-colors"
                variants={itemAnimation}
                animate="float"
                whileHover={{
                  scale: 1.05,
                  backgroundColor: "rgba(139, 208, 5, 0.2)",
                }}
                transition={{
                  float: {
